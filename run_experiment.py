...
parser.add_argument('--seed', type = int, default = 100, help = "Seed for random number generator.")
...
if __name__ == '__main__':
    ...
    for i in range(args.num_experiments):
        ...
        target = circuit_parser(args.target_circuit, nqubits = args.num_qubits, depth = args.depth, entanglement = args.entanglement, rng = rng, is_target = True)
        anstaz = circuit_parser(args.ansatz_circuit, nqubits = args.num_qubits, depth = args.depth, entanglement = args.entanglement, rng = rng, is_target = False)
        gen = Generator(ansatz = anstaz, rng = rng)
        
        print("\nIteration ", i, "Method: ", args.method, "\n")
            
        gen_fixed = gen.compile(target_qc = target, method = args.method, 
                                test_states = args.test_states, T = args.time_steps,
                                verbose = args.time_steps // 2,
                                initialization_argument = args.type_of_states, 
                                optimizer_hyper = dict(optimizer = args.optimizer, 
                                                        lr = args.learning_rate),
                                compiler_hyper = dict(m = args.operator_cycling, 
                                                        k = args.k_locality, 
                                                        s = args.num_states, 
                                                        N_s = args.state_pool, 
                                                        local = args.locality, 
                                                        cnvgc_threshold = args.convergence_threshold, 
                                                        cnvgc_break = args.convergence_break), )
        
        gen_fixed.dump('runs', prefix = f'Run{i}_{args.method}-met_{args.num_states}states_')